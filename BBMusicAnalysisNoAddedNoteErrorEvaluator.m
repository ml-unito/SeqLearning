...
#import "BBMusicAnalysisNoAddedNoteErrorEvaluator.h"
#import <SeqLearning/BBMusicAnalysis.h>
#import <SeqLearning/BBMutableDouble.h>


static BOOL bb_music_labels_differ_no_added_note( NSString* predicted, NSString* correct ) {
...
@implementation BBMusicAnalysisNoAddedNoteErrorEvaluator

-(BOOL) prediction:(NSString*) predicted differsFrom:(NSString*) correct {
	return bb_music_labels_differ_no_added_note(predicted, correct);
}
...
-(double) evaluateErrorUsingLabelledSeqSet:(BBSeqSet*) ss {
	...
	while( sequence = [seq_enumerator nextObject] ) {
		...
		for(n=0; n<seq_len; ++n) {
			...
			if( bb_music_labels_differ_no_added_note(predicted, correct) ) {
				++error;				
