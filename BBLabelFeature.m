...
NSString* BBLabelFeatureKeyChordName = @"Chord";

@implementation BBLabelFeature

-(BOOL) evalOnSequence:(BBSequence*) sequence forTime:(unsigned int) t {
	NSObject* cur_label = [sequence labelAtTime:t];
	NSObject* target_label = [_parameters objectForKey:BBLabelFeatureKeyChordName];
	return [cur_label isEqual:target_label];
}
